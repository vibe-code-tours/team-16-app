...
export function UserProfile() {
  ...
  const learnerName = user?.display_name?.trim() || email?.split('@')[0] || 'NerdQuiz learner'
  const initials = getInitials(learnerName)
  const totalXp = user?.total_xp ?? 0
  const streakCount = user?.streak_count ?? 0
  const levelProgress = getLevelProgress(totalXp)
  const avatarUrl = user?.avatar_url?.startsWith('http') ? user.avatar_url : null
  ...
  async function handleSaveName(event: FormEvent<HTMLFormElement>) {
    ...
    setSaving(true)
    setError(null)
    const { error: updateError } = await updateProfile({ display_name: trimmedDisplayName })
...
function ProfileHero({
  ...
  return (
    <section className="learning-feature-enter relative overflow-hidden rounded-3xl bg-gradient-to-br from-violet-700 via-purple-600 to-fuchsia-600 p-6 text-white shadow-xl shadow-purple-200/60 sm:p-8">
      <div className="absolute -right-16 -top-20 size-64 rounded-full bg-white/10 blur-sm" aria-hidden="true" />
      <div className="absolute -bottom-28 right-1/4 size-56 rounded-full bg-fuchsia-300/20 blur-2xl" aria-hidden="true" />
      <div className="absolute right-10 top-8 hidden rotate-12 text-white/10 lg:block" aria-hidden="true">
      ...
      <div className="relative grid gap-7 lg:grid-cols-[minmax(0,1fr)_minmax(300px,0.55fr)] lg:items-center">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
          <div className="relative w-fit shrink-0">
            ...
            <span className="absolute -bottom-1 -right-1 inline-flex size-9 items-center justify-center rounded-full bg-amber-400 text-amber-950 shadow-md ring-4 ring-purple-600" title={`Level ${levelProgress.level}`}>
              <Icon name="star" className="size-4 fill-current" aria-hidden="true" />
          ...
          <div className="min-w-0 flex-1">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1 text-xs font-semibold ring-1 ring-white/20 backdrop-blur-sm">
              <Icon name="shield-check" className="size-3.5" aria-hidden="true" />
              ...
              <form className="mt-3" onSubmit={onSaveName}>
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {saving ? <span className="size-4 animate-spin rounded-full border-2 border-purple-300 border-t-purple-700" aria-hidden="true" /> : <Icon name="save" className="size-4" aria-hidden="true" />}
                      {saving ? 'Saving' : 'Save'}
                    </button>
                    <button
                      type="button"
                ...
                {error ? <p id="display-name-error" role="alert" className="mt-2 text-sm font-medium text-rose-100">{error}</p> : null}
              </form>
            ) : (
              <div className="mt-3 flex flex-wrap items-center gap-3">
                <h2 className="truncate text-2xl font-bold sm:text-3xl">{displayName}</h2>
                <button
                  type="button"
                  onClick={onStartEdit}
            ...
            <p className="mt-1 truncate text-sm text-purple-100">{email ?? 'Email not available'}</p>
            <p className="mt-3 max-w-xl text-sm leading-6 text-white/80">
              Curious mind, steady progress. You&apos;re building exam-ready confidence one lesson at a time.
            </p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-white/20 bg-white/10 p-5 backdrop-blur-md">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-purple-100">Next level</p>
              <p className="mt-1 text-xl font-bold">{levelProgress.remainingXp} XP to go</p>
            </div>
          ...
          <div className="mt-5 flex items-center justify-between text-xs font-medium text-purple-100">
            ...
            <span>{levelProgress.currentXp} / {levelProgress.xpPerLevel} XP</span>
          </div>
          <div
          ...
          >
            <div className="learning-progress-reveal h-full rounded-full bg-gradient-to-r from-amber-300 to-yellow-200" style={{ width: `${levelProgress.percent}%` }} />
          </div>
          <p className="mt-3 text-xs leading-5 text-white/70">Complete a quiz today to move your progress forward.</p>
        </div>
      </div>
    </section>
  )
...
function StatsGrid({ totalXp, streakCount }: { totalXp: number; streakCount: number }) {
  const stats: StatItem[] = [
    { label: 'Total XP', value: totalXp.toLocaleString(), detail: '+120 this week', icon: 'zap', color: 'amber' },
    { label: 'Day streak', value: `${streakCount} days`, detail: 'Personal best: 7', icon: 'flame', color: 'orange' },
    { label: 'Quizzes', value: '24', detail: '18 completed', icon: 'book-open', color: 'purple' },
    { label: 'Accuracy', value: '86%', detail: '+8% this month', icon: 'target', color: 'emerald' },
  ]
  ...
  return (
    <section className="learning-feature-enter rounded-3xl border border-gray-200/80 bg-white p-5 shadow-sm sm:p-6" aria-labelledby="account-title">
      ...
      <dl className="mt-5 space-y-4">
        <AccountDetail icon="mail" label="Email" value={email ?? 'Not available'} />
...
function Icon({ name, ...props }: IconProps) {
  ...
  switch (name) {
    ...
    case 'clock':
      ...
      break
    case 'flame':
      content = <path d="M12 22c4.4 0 8-3.4 8-7.8 0-3.2-1.7-6.1-4.7-8.7.1 2.1-.8 3.7-2.1 4.6.2-3.7-1.7-6.4-4.6-8.1.2 3-1.5 5.1-3.1 7.1C4.1 10.8 4 12.6 4 14.2 4 18.6 7.6 22 12 22Z" />
      break
    case 'log-out':
      content = (
        <>
    ...
    case 'sparkles':
      ...
      break
    case 'star':
      content = <path d="m12 2.8 2.8 5.7 6.3.9-4.5 4.4 1 6.2-5.6-3-5.6 3 1-6.2-4.5-4.4 6.3-.9Z" />
      break
    case 'target':
      content = (
        <>
