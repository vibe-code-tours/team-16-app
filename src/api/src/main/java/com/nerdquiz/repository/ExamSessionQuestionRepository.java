...
public interface ExamSessionQuestionRepository extends JpaRepository<ExamSessionQuestion, ExamSessionQuestionId> {
    ...
    @Query("SELECT q FROM ExamSessionQuestion q WHERE q.id.examSessionId = :examSessionId AND q.id.questionId = :questionId")
    Optional<ExamSessionQuestion> findIssuedQuestion(@Param("examSessionId") UUID examSessionId,
