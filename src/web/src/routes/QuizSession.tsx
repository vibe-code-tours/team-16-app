...
export function QuizSession() {
  ...
  const handleNextQuestion = useCallback(() => {
    ...
    if (isLastQuestion) {
      ...
      result.score_percentage = Math.round(
        (result.correct_answers / result.total_questions) * 100
      );
      result.xp_earned = result.correct_answers * 10;
  ...
  }, [currentIndex, isLastQuestion, answers, questions, navigate]);
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-4 text-sm text-gray-600">
              ...
              <span>XP: {Object.values(answers).filter((a) => a.submitted && a.selected === questions[parseInt(Object.keys(answers).find((k) => answers[parseInt(k)] === a) || '0')]?.correct_answer).length * 10}</span>
            </div>
          </div>
          <ProgressBar
